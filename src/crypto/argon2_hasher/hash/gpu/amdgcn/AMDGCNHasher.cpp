...
#include "AMDGCNHasher.h"
#include "AMDGCN_Kernels.h"
...
bool AMDGCNHasher::initialize(xmrig::Algo algorithm, xmrig::Variant variant) {
    cl_int error = CL_SUCCESS;
    string error_message;

    m_profile = getArgon2Profile(algorithm, variant);

    m_devices = queryAMDGCNDevices(error, error_message);
    if(error != CL_SUCCESS) {
...
vector<AMDGCNDeviceInfo*> AMDGCNHasher::queryAMDGCNDevices(cl_int &error, string &error_message) {
    cl_int err;

    cl_uint device_count = 0;

    vector<AMDGCNDeviceInfo*> result;
    ...
    for(uint32_t i=0; i < platforms.size(); i++) {
        ...
        for(uint32_t j=0; j < device_count; j++) {
            AMDGCNDeviceInfo *info = getDeviceInfo(platforms[i], devices[j]);
...
AMDGCNDeviceInfo *AMDGCNHasher::getDeviceInfo(cl_platform_id platform, cl_device_id device) {
    AMDGCNDeviceInfo *device_info = new AMDGCNDeviceInfo(CL_SUCCESS, "");
    ...
    string device_name;
    cl_device_info query_type = CL_DEVICE_BOARD_NAME_AMD;
    ...
    device_info->deviceString = device_name;
...
bool AMDGCNHasher::configure(xmrig::HasherConfig &config) {
    ...
    for(vector<AMDGCNDeviceInfo *>::iterator d = m_devices.begin(); d != m_devices.end(); d++, index++) {
        stringstream ss;
...
bool AMDGCNHasher::setupDeviceInfo(AMDGCNDeviceInfo *device, double intensity) {
    cl_int error;
    ...
    try {
        ...
        device->errorMessage = string("Error retrieving assembler setup: ") + ex.what();
        return false;
    }
    ...
    const char *srcptr[] = {AMDGCN_OCL_Kernels.c_str()};
    size_t srcsize = AMDGCN_OCL_Kernels.size();

    device->programOpenCL = clCreateProgramWithSource(device->context, 1, srcptr, &srcsize, &error);
    if (error != CL_SUCCESS) {
        device->error = error;
        device->errorMessage = "Error creating AMDGCN - OCL program for device.";
        return false;
    }
    ...
    error = clBuildProgram(device->programOpenCL, 1, &device->device, options.c_str(), NULL, NULL);
    if (error != CL_SUCCESS) {
        size_t log_size;
        clGetProgramBuildInfo(device->programOpenCL, device->device, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);
        char *log = (char *) malloc(log_size + 1);
        clGetProgramBuildInfo(device->programOpenCL, device->device, CL_PROGRAM_BUILD_LOG, log_size, log, NULL);
        log[log_size] = 0;
        string build_log_ocl = log;
        free(log);

        device->error = error;
        device->errorMessage = "Error building AMDGCN - OCL program for device: " + build_log_ocl;
        return false;
    }

    CLRX::CString build_log_gcn;
    try
    {
        vector<CLRX::CString, uint64_t> defs;
        defs.push_back("SHARED_MEM_SIZE", 2 * device->profileInfo.profile->thrCost * ARGON2_BLOCK_SIZE);
        device->programAMDGCN = CLRX::createProgramForCLDevice(device->context, device->device, defs,
                                                               asmSetup, AMDGCN_GCN_Kernels.c_str(), 0, &build_log_gcn);
    }
    catch(const CLRX::Exception &ex)
    {
        device->error = CL_INVALID_BINARY;
        device->errorMessage = "Error building AMDGCN - GCN program for device: " + string(build_log_gcn.c_str());
        return false;
    }

    device->kernelPrehash[0] = clCreateKernel(device->programOpenCL, "prehash", &error);
    if (error != CL_SUCCESS) {
        device->error = error;
        device->errorMessage = "Error creating AMDGCN prehash kernel for device.";
        return false;
    }
    device->kernelPrehash[1] = clCreateKernel(device->programOpenCL, "prehash", &error);
    if (error != CL_SUCCESS) {
        device->error = error;
        device->errorMessage = "Error creating AMDGCN prehash kernel for device.";
        return false;
    }
    device->kernelFillBlocks[0] = clCreateKernel(device->programAMDGCN, "fill_blocks", &error);
    if (error != CL_SUCCESS) {
        device->error = error;
        device->errorMessage = "Error creating AMDGCN main kernel for device.";
        return false;
    }
    device->kernelFillBlocks[1] = clCreateKernel(device->programAMDGCN, "fill_blocks", &error);
    if (error != CL_SUCCESS) {
        device->error = error;
        device->errorMessage = "Error creating AMDGCN main kernel for device.";
        return false;
    }
    device->kernelPosthash[0] = clCreateKernel(device->programOpenCL, "posthash", &error);
    if (error != CL_SUCCESS) {
        device->error = error;
        device->errorMessage = "Error creating AMDGCN posthash kernel for device.";
        return false;
    }
    device->kernelPosthash[1] = clCreateKernel(device->programOpenCL, "posthash", &error);
    if (error != CL_SUCCESS) {
        device->error = error;
        device->errorMessage = "Error creating AMDGCN posthash kernel for device.";
        return false;
...
bool AMDGCN_kernel_prehasher(void *memory, int threads, Argon2Profile *profile, void *user_data) {
    AMDGCNGpuMgmtThreadData *gpumgmt_thread = (AMDGCNGpuMgmtThreadData *)user_data;
    AMDGCNDeviceInfo *device = gpumgmt_thread->device;
...
void *AMDGCN_kernel_filler(int threads, Argon2Profile *profile, void *user_data) {
    AMDGCNGpuMgmtThreadData *gpumgmt_thread = (AMDGCNGpuMgmtThreadData *)user_data;
    AMDGCNDeviceInfo *device = gpumgmt_thread->device;
...
bool AMDGCN_kernel_posthasher(void *memory, int threads, Argon2Profile *profile, void *user_data) {
    AMDGCNGpuMgmtThreadData *gpumgmt_thread = (AMDGCNGpuMgmtThreadData *)user_data;
    AMDGCNDeviceInfo *device = gpumgmt_thread->device;
...
void AMDGCNHasher::buildThreadData() {
    m_threadData = new AMDGCNGpuMgmtThreadData[m_enabledDevices.size() * 2];

    for(int i=0; i < m_enabledDevices.size(); i++) {
        AMDGCNDeviceInfo *device = m_enabledDevices[i];
        for(int threadId = 0; threadId < 2; threadId ++) {
            AMDGCNGpuMgmtThreadData &thread_data = m_threadData[i * 2 + threadId];
            thread_data.device = device;
            thread_data.threadId = threadId;

#ifdef PARALLEL_AMDGCN
            if(threadId == 0) {
            ...
            thread_data.argon2 = new Argon2(AMDGCN_kernel_prehasher, AMDGCN_kernel_filler, AMDGCN_kernel_posthasher,
                                            nullptr, &thread_data);
...
int AMDGCNHasher::compute(int threadIdx, uint8_t *input, size_t size, uint8_t *output) {
    AMDGCNGpuMgmtThreadData &threadData = m_threadData[threadIdx];
...
void AMDGCNHasher::cleanup() {
    vector<cl_platform_id> platforms;

    for(vector<AMDGCNDeviceInfo *>::iterator it=m_devices.begin(); it != m_devices.end(); it++) {
        if ((*it)->profileInfo.threads != 0) {
            ...
            clReleaseProgram((*it)->programOpenCL);
            clReleaseProgram((*it)->programAMDGCN);
...
size_t AMDGCNHasher::parallelism(int workerIdx) {
    AMDGCNGpuMgmtThreadData &threadData = m_threadData[workerIdx];
    return threadData.threads;
}

size_t AMDGCNHasher::deviceCount() {
    return m_enabledDevices.size();
}

DeviceInfo &AMDGCNHasher::device(int workerIdx) {
    workerIdx /= 2;
